...
var ALLOWED_CLOUD_CONFIG = ["cloudName", "apiKey", "apiSecret", "authToken"];
...
var ResizeSimpleAction = class extends Action {
  ...
  static fromJson(actionModel) {
    const {
      ...
      dimensions,
      relative: relative2,
      regionRelative: regionRelative2
    } = actionModel;
    const {
      aspectRatio,
      width,
...
var PreviewAction = class extends Action {
  ...
  toString() {
    return ["e_preview", this._duration && `duration_${toFloatAsString(this._duration)}`, this._maxSeg && `max_seg_${this._maxSeg}`, this._minSeg && `min_seg_dur_${toFloatAsString(this._minSeg)}`].filter((a) => a).join(":");
  }
  static fromJson(actionModel) {
    const {
      duration,
...
function processCustomFunction3({
  function_type,
  source: source3
}) {
  ...
  if (isAnyChildAnObject) {
    childTransformations = childTransformations.map((transformation) => {
      if (isObject5(transformation)) {
        ...
        return generateTransformationString2({
          transformation
        });
      }
    }).filter((a) => a);
...
//# sourceMappingURL=chunk-VM6TNYGA.js.map
