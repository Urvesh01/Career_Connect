...
const InquirerWrapper_1 = require("./InquirerWrapper");
class BasePromptSession {
    ...
    getUserInput(options_1) {
        return __awaiter(this, arguments, void 0, function* (options, withBackChoice = false) {
            ...
            if (options.type === "list") {
                result = yield InquirerWrapper_1.InquirerWrapper.select(options);
            }
            else {
                result = yield InquirerWrapper_1.InquirerWrapper.input(options);
            }
     ...
     */
    chooseTemplateName(template_1) {
        return __awaiter(this, arguments, void 0, function* (template, type = "component") {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (const question of extraPrompt) {
                switch (question.type) {
                    case "input":
                        extraConfigAnswers.push(yield InquirerWrapper_1.InquirerWrapper.input(question));
                        break;
                    case "select":
                        extraConfigAnswers.push(yield InquirerWrapper_1.InquirerWrapper.select(question));
                        break;
                    case "checkbox":
                        extraConfigAnswers.push(yield InquirerWrapper_1.InquirerWrapper.checkbox(question));
                        break;
                }
            }
    ...
    addSeparators(array) {
        ...
        for (let i = 0; i < array.length; i++) {
            ...
            if (i + 1 < array.length) {
                newArray.push(new prompts_1.Separator());
            }
        }
        if (array.length > 4) {
            // additional separator after last item for lists that wrap around
            newArray.push(new prompts_1.Separator(new Array(15).join("=")));
        }
        return newArray;
    }
    /**
    ...
    createQuestions(extraConfig) {
        ...
        for (const element of extraConfig) {
            ...
            switch (element.type) {
                case 0 /* ControlExtraConfigType.Choice */:
                    currExtraConfig["type"] = "select"; // formerly list
                    break;
                case 1 /* ControlExtraConfigType.MultiChoice */:
                    currExtraConfig["type"] = "checkbox";
                    break;
                case 2 /* ControlExtraConfigType.Value */:
                default:
